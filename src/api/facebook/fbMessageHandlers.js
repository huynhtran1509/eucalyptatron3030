 ...
 */
// API
import query from '../../luis/query';
import mnlpQuery from '../../magic-nlp/query';
import { path, prop, concat } from 'ramda';
import { sendTextPayload } from '../../facebook/send';
import handleGreetingIntent from './intents/greeting';
import handleFarewellIntent from './intents/farewell';
import handleShowtimeIntent from './intents/showtime';
import handleSettingsIntent from './intents/settings';
import handleRCCIntent from './intents/rcc';
import handleMovieInfoIntent from './intents/movieInfo';
import { sendSettingsLatLng } from './send/settings';
import { sendMovieDescription, sendWatchTrailer } from './postbacks/movie';
import { requestZip, setMovieTheatre, viewSettings } from './postbacks/settings';
import { sendCatchAll } from '../../regal/facebook/send/catchall';
import { randomResponse, listOfDoNotUnderstandQueryResponses, listOfUserHitYesResponses, listOfUserHitNoResponses, listOfDoNotUnderstandRequestedPayloadResponses, getStartedShareLocationPrompt } from './intents/responseContent';
import { IMessagingReceived, IMessagingPostback } from '../../../handlers/facebook/webhook-interface';
import { showLocationQuickReplyButton, showQuickReplyForErrorHandling } from './send/quickReply';
import { getTheatreLocationsLatLng } from '../query';
import { sendSingleMoviePayload, sendMoviePayload } from '../../regal/facebook/send/movies'
import { sendShowtimesPayload } from '../../regal/facebook/send/showtimes'


export function messageReceivedHandler(message: IMessagingReceived) {
    ...
    return query(LUIS_API_ROOT, text).then((luisData) => {
        return mnlpQuery({
            'user-id': {
                'type': 'facebook',
                ID: senderId
            ...
            if (mnlpData['Showtimes']) {
                ...
                const performances = [].concat.apply([], mnlpData['Showtimes'].map( function(obj) { return obj["Performances"] }));
                return Promise.all([
                        sendMNLPUtterance(senderId, mnlpData.utterance),
